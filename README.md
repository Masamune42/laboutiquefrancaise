...
1. On modifie Order.php : on crée le champ state et on retire le champ isPaid

2. On migre la BDD

3. On change dans le OrderController() et le OrderSuccessController() la classe isPaid() par setState()

4. Dans le OrderCrudController on modifie la fonction configureActions() et on ajoute updatePreparation()
```php
